...
def login():
    ...
    if not data:
        return jsonify({'success': False, 'message': 'Solicitud vacía'}), 400
    ...
    try:
        ...
        if usuario:
            ...
            if bcrypt.checkpw(contraseña.encode('utf-8'), usuario['contraseña'].encode('utf-8')):
                ...
                with open('usuarios_login.json', 'a') as json_file:
                    json_file.write(json.dumps(usuario_data) + '\n')

                return jsonify({'success': True, 'message': 'Inicio de sesion exitoso'})
            else:
                return jsonify({'success': False, 'message': 'Datos incorrectos!'}), 500
        else:
            return jsonify({'success': False, 'message': 'Usuario no registrado'}), 500

    except mysql.connector.Error as err:
        return jsonify({'success': False, 'message': f'Error al iniciar sesion: {err}'}), 500
...
def registro():
    ...
    try:
        ...
        hashed_password = bcrypt.hashpw(
            contraseña.encode('utf-8'), bcrypt.gensalt())
        ...
        return jsonify({'success': True, 'message': 'Usuario registrado con exito'})

    except mysql.connector.Error as e:

        return jsonify({'success': False, 'message': f'Error al registrar el usuario: {e}'}), 500
    finally:
        cursor.close()
...
def pedidos_finalizado():
    ...
    try:
        ...
        cursor.execute(
            'UPDATE pedidos SET estado = %s WHERE id = %s', ('entregado', pedido_id))
...
def pedido_cancelado():
    ...
    try:
        ...
        cursor.execute(
            'UPDATE pedidos SET estado = %s WHERE id = %s', ('cancelado', pedido_id))
        conexion.commit()
...
def admin_productos_save():
    ...
    datos = (_nombre, _descripcion, _precio, _stock, _categoria,
             nuevoNombre if nuevoNombre else None, fecha_actual, fecha_actual)
...
def agregar_carrito():
    ...
    try:
        ...
        cursor.execute(
            'SELECT stock FROM productos WHERE id = %s', (producto_id,))
        producto = cursor.fetchone()
        ...
        if cantidad > stock_disponible:
            flash(f'No hay suficiente stock. Solo hay {
                  stock_disponible} unidades disponibles.', 'danger')
            return redirect('/cliente/inicio_cliente')

        cursor.execute(
            'SELECT * FROM carritos WHERE usuario_id = %s', (session['id'],))
...
def delete_carrito():
    ...
    if eliminar_todo == 'true':
        cursor.execute(
            'DELETE FROM carrito_productos WHERE carrito_id=%s', (id_carrito,))
        ...
        if resultado and resultado['cantidad'] > cantidad:
            ...
            flash(f"Se eliminaron {
                  cantidad} unidades del producto.", "success")
        else:
            cursor.execute(
...
def pagar():
    ...
    if request.method == 'POST':
        ...
        if not carrito_id or not metodo_pago or not direccion_envio:
            flash(
                'Todos los campos son obligatorios. Por favor, complete la información.', 'warning')
        ...
        try:
            ...
            ''', (carrito_id,))
            cursor.execute(
                'DELETE FROM carrito_productos WHERE carrito_id = %s', (carrito_id,))
